...
# shape=[20, 60], num_candidates=10, threshold=0.11
def ptcloud2sc(ptcloud, sc_shape, max_length):
    ...
    for pt_idx in range(num_points):
        ...
        point_height = point[2] + 1.7  #  2.0  # for setting ground is roughly zero
...
class ScanContextManager:
    #  default configs are same as the original paper
    def __init__(self, shape=[20, 60], num_candidates=10, threshold=0.15):
        self.shape = shape
        self.num_candidates = num_candidates
        self.threshold = threshold

        self.max_length = 50 # 80  # recommended but other (e.g., 100m) is also ok. 50 for indoor
